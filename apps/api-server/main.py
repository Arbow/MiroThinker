...
async def deep_research(request: DeepResearchRequest):
    """
    Multi-round deep research using MiroThinker Agent.
    
    Uses the real MiroThinker Agent with Hydra configuration and Tavily MCP server.
    The max_search_rounds parameter controls the maximum number of tool calls (searches).
    ...
    try:
        ...
        print(f"🤖 Starting deep research with MiroThinker Agent: {request.query}")
        print(f"📊 Max tool calls: {request.max_search_rounds}")
...
**Query**: {request.query}
**Tool Call Rounds**: {result.get('search_rounds', 0)}
